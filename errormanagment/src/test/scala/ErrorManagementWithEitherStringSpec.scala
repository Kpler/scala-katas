...
class ErrorManagementWithEitherStringSpec extends AnyFlatSpec with Matchers {
  ...
  "Withdrawing more money than the bank account holds" should "return Left with error message " in {
    ...
    withDraw(amount = 20_000, bankAccount = emptyBankAccount) shouldBe Left(
      "amount should be lower that available money"
    )
  }
  ...
  "(using flatMap) multiple cash withdrawals  must work with an adequately funded account" should "return true" in {
    ...
    val bankAccountWithTwoWithdraw: Either[String, BankAccount] =
      bankAccountOneWithdrawOrError.flatMap(bankAccount => withDraw(100, bankAccount))
    bankAccountWithTwoWithdraw shouldBe Right(BankAccount(9_800, Open))

    val bankAccountWithThreeWithdraw: Either[String, BankAccount] =
      bankAccountWithTwoWithdraw.flatMap(bankAccount => withDraw(100, bankAccount))
